...
def _get_schemas_dir() -> Path:
    local = Path(__file__).resolve().parents[2] / "schemas"
    if local.is_dir():
        return local
    try:
        import importlib.resources as resources

        pkg = resources.files("ghost_channel_sdk")
        sd = pkg / "schemas"
        if sd.is_dir():
            return sd
    except Exception:
    ...
    raise FileNotFoundError(
        ...
        "When using pip install without editable mode, ensure schemas/ is copied "
        "alongside ghost_channel_sdk/, or run 'pip install -e .' from the python/ directory."
    )
